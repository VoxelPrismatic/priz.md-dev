...
;var std_err__ = [[/([\w\d_:]*)([Ee]rror|[Ee]xception|[Ff]ailure|[Ee]xit|[Ww]arning)/gm,
...
kw.sort().reverse();cls.sort().reverse();var sym = "[\\"+"\\.,:;()[]{}~|/-+=*^%&@ ".split('').join("\\")+"]";var gsym = "(" + sym + ")";if(edit) {for(var r of kw) {if(r == "class") {r += "[^=]";}
...
if(aio) {for(var r of aio_text) {st = st.replace(RegExp("^" + r + gsym, "gm"),
