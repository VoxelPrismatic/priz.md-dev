 ...
 */
var html_regex = [[/([^\\])<((.|\n)+?)>/gm,
function(m, p2, p1) {st = "";if(p1.startsWith("!--") && p1.endsWith("--")) {return `${p2}<span class="com">&lt;${p1}$gt;</span>`}
st = `${p2}<span class="op">&lt;${p1.split(" ")[0]}</span>`;for(var attr of p1.split(" ").slice(1)) {st += " ";if(attr.includes("=")) {st += `<span class="kw">${attr.split("=")[0] + "="}</span>`;st += `<span class="str">${attr.split("=")[1]}</span>`;}
else {st += `<span class="str">${attr.split("=")[0]}</span>`;}}
