 ...
 */
var cls = ["int", "float", "dict", "list", "tuple", "set", "bool", "None", "frozenset",
"str", "bytes", "file", "type", "complex", "True", "range", "bytearray",
"memoryview", "False", "range", "breakpoint", "callable", "classmethod",
"map", "object", "quit"]
;var kw = ["import", "from", "yield", "break",  "if", "assert",  "finally", "def",
...
py_str_regex], [/([fFrRuUbB]?)(""")((.|\n)*)"""/gm,
py_str_regex], [/\\u([A-Fa-f0-9\u200b]{8})/gm,
`<span class="op">\\u$1</span>`], [/\\U([A-Fa-f0-9\u200b]{16})/gm,
`<span class="op">\\U$1</span>`], [/\\x([A-Fa-f0-9\u200b]{4})/gm,
`<span class="op">\\x$1</span>`], [/\\N\{([\w\d ]+)\}/gm,
`<span class="op">\\N{$1}</span..,>`], [/\\(.)/gm,
`<span class="op">\\$1</span>`], [/^([\u200b ]*)def ([\w\d_]+)/gm,
...
, [/\u200b/gm,
""]]
;function mark_syntax_py(st) {st = st.replace(/\n/gm, " \n");var sym = "[\\"+"\\.,:;()[]{}~|/-+=*^%&@ ".split('').join("\\")+"]";var gsym = "("+sym+")";for(var r of py_regex) {st = st.replace(r[0], r[1]);}
...
for(var r of kw) {st = st.replace(RegExp("^"+r+gsym, "gm"),
