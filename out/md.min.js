...
if(!(st.endsWith("\n")))
st+="\n";st=st.replace(/\\ *\n/gm,"");var str="";var py="";var table="";var ol="";var ul="";var quoted="";var incode=false;var intable=false;for(var line of st.split("\n")) {if(line=="```") {incode=!incode;if(incode)
...
n+=1;}}}}}
function resizeDicts(log=true,times=0) {if((window.innerHeight+""+window.innerWidth).includes("."))
...
[/\\ *$/gm,"</br>"],[/^-~-$/gm,"<div class='line'></div></br>"],
[/(<u>_<\/u>|___)/gm,"<div>"],
[/===/gm,"</div>"],
[/\\u\{([a-fA-F0-9]+)\}/gm,
...
return "&gt;"
return st;}],];}
function mk_head(m,p1) {for(var x=0; x < 6; x+=1)
if(m[x] !="#")
break;var id=p1.replace(/[^\w\d]/gm,"").trim();var st=`<h${x} onclick="linkMe(this);"`;st+=`id="${id}">\\u{23}] ${p1}</h${x}>`;return st;}
...
function mk_ul(st,thing="ul") {var str=`<${thing}>`;st=mark_page(st);for(var line of st.split("<br>")) {if(line) {var extra="";if(line.startsWith("<ul>")||line.startsWith("<ol>")) {extra=' class="hidden-marker"';}
