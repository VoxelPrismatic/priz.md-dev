 ...
 */
function mark(st){for(var r of line_regex){if(typeof r[1]=="string"&&!(r[1].startsWith("\\u")))
st=st.replace(r[0],r[1]);else
st=st.replace(r[0],r[1]);}
return st;}
function mark_page(st){if(!line_regex.length){set_regex();}
if(!(st.endsWith("\n")))
st+="\n";st=st.replace(/\\*\n/gm,"");var str="";var py="";var table="";var ol="";var ul="";var quoted="";var dropper="";var incode=false;var indropper=false;var intable=false;for(var line of st.split("\n")){
if(!incode&&line&&line.replace(/^\>\>\[.*\]\<\<$/gm,"")==""){indropper=true;}
if(!incode&&line=="---"&&indropper){indropper=false;str+=mk_dropper(dropper.slice(0,-1));dropper="";continue;}
if(indropper){dropper+=line+"\n";continue;}
if(line=="```"){incode=!incode;if(incode)
str+=`<div class="code">`;else
str+=`</div>`
continue;}
if(incode){str+=line+"\n";continue;}
...
function style_accent(){for(var acc of find(".accent")){var char=acc.previousElementSibling;char.style.display="inline-block";acc.style.position="relative";var ok_accents=["\u0300",
"\u0301",
"\u0302",
"\u030c",
"\u0303",
...
n+=1;}}}}}
var line_regex=[];function set_regex(){line_regex=[[/\\([^uUNx])/gm,function(m,p){return "\\u{"+p.charCodeAt(0).toString(16)+"}";}],
[/^/gm,"\u200b \u200b"],
[/\&gt;/gm,">"],
[/\&lt;/gm,"<"],
[/\&amp;/gm,"&"],
...
"-]": "\u0321",
"-[": "\u0322"}
;function mk_head(m,p1,escape=true,cls=""){if(cls==m){cls="";}
...
function mk_ul(st,thing="ul"){var str=`<${thing}>`;st=mark_page(st);for(var line of st.split("<br>")){if(line){if(line.startsWith("<ul>")||line.startsWith("<ol>")){str=str.slice(0,-5);str+=line+"</li>";}
else {str+=`<li>`+line+"</li>";}}}
return str+`</${thing}><br>`;}
